# Claude Instructions for vibe_proj
...
## ⚠️ CRITICAL: No Installing Dependencies Without Explicit Permission

**Never run a package manager install command (`pip install`, `npm install`, `pnpm install`, `pnpm add`, `yarn add`, `cargo add`, `gem install`, etc.) without the user's explicit go-ahead first.**

- If a dependency appears to be missing, **stop and report it** — name the missing package and where you observed the gap — rather than installing it yourself.
- This also covers commands that *silently* install as a side effect: `pnpm exec`, `pnpm run`, `npm exec`/`npx`, and similar "run a script" commands will transparently reconcile `node_modules` against the lockfile before running anything. If dependencies are missing, running one of these can install packages even though you never typed an install command. Treat a missing `node_modules` (or equivalent) as a stop condition, not something to route around by running the test/build command anyway.
- Exception: commands the user has explicitly pre-authorized (e.g. via a documented project workflow) are fine to run as documented.

**Why**: Dependency installation changes what's on disk in ways that are easy to miss and hard to fully reverse (version drift, lockfile changes, transitive packages). The user needs to decide when and how dependencies get installed — not have it happen as a side effect of "just running the tests."
...
- `.xgd/machines.yaml` (multi-machine registry)
...
- `.xgd/machine_id`, `.xgd/regression.state.json`, `.xgd/stats/` (machine-local or per-worktree runtime state — meaningless once synced elsewhere, must never ride along on a merge/sync)

**Editing `.xgd/config.yaml`, `.xgd/quality.yaml`, or `.xgd/surfaces.yaml`**: these three are main-only (REQ-709) — edit them through the dashboard (Files tab, or Quality tab's plugin apply) or the relevant `xgd quality config` subcommand. The tool resolves and commits to the standing main worktree automatically, regardless of which worktree/branch you're running from — no manual "switch to main" step. Editing the copy checked out in your *own* branch worktree directly (e.g. with a raw file edit) has no effect: it's an inert local mirror, not the file the tool reads from. This is different from adding a new config.yaml *key* to the schema: that goes through the template (`xgd_source/templates/files/config.yaml`), not a live edit — see `xgd_source/system_docs/CODING-STANDARDS.md` → "Configuration Keys — Template, Not Live Config".

`.xgd/machine_id` / `.xgd/machines.yaml` are the opposite: genuinely per-machine (distinct hostname/ip per REQ-589), not main-only — each machine registers itself via its own git commit + push, never edited by hand.
